...
export async function getAllProductionsByType(type, preview = false) {
  const data = await fetchAPI(
    `
  query getAllEstampidas ($stage: Stage!, $type: ProjectType!) {
    productions(stage: $stage, where: {projectType: [$type]}){
      id
      title
      description
      projectType
...
export async function getProductionById(id, preview = false) {
  const data = await fetchAPI(
    `query getProductionById($id: ID!, $stage: Stage!){
        production(stage: $stage, where:{id:$id}){
          id
          title
          description
        }
